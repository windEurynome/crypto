...
'''
 function: use rsa to decrypto the message
'''
...
def test():
    ...
    str, pk = rsaEncrypt("000102030405060708090a0b0c0d0e0f")
    print('\n')
    print("AES密钥已加密存放到p_keyde.txt中。")
    str1=binascii.hexlify(str)
    f = open('E:/0000桌面需要小文件/grade3/密码学课程设计/p_keyde.txt','a')
    #can change the file path,'a'means to continue the last ones
    print(bytesToString(str1),end='',file = f)
    #print(bytesToString(str1),end='')
    f.close()
    #print('\n')
    #print(str1)
    #print(pk)

    str=binascii.unhexlify(str1)
    content = rsaDecrypt(str, pk)
    print('\n')
    print('解密后明文：')
    f = open('E:/0000桌面需要小文件/grade3/密码学课程设计/p_key.txt','a')
    ...
    print("***********************AES-128***********************")
    print("***********************RSA密文***********************")
    with open('E:/0000桌面需要小文件/grade3/密码学课程设计/p_keyde.txt','r',encoding='utf-8') as f:
        key = f.read()
    #print(key)
    print("从p_keyde.txt读取被加密的AES密钥。")
    print('\n')
    print("************************密文**************************")
    #print("PLAINTEXT:        %s" % binascii.hexlify(plaintext))
    #print("KEY:              %s" % binascii.hexlify(key128))
    print("密文存放在p_encrypto.txt中。")
    ...
    for KEYS in string1:
        ...
        #B2S=cipher128
        #file_handle=open('F:\out.txt','w')

        #print(binascii.hexlify(cipher128))
        cipher=binascii.hexlify(cipher128)
        f = open('E:/0000桌面需要小文件/grade3/密码学课程设计/p_encrypto.txt','a')
        #can change the file path,'a'means to continue the last ones
        print(bytesToString(cipher),end='',file = f)
        #print(bytesToString(cipher),end='')
        f.close()
        #print("密文已经存放在p_encrypto.txt中。")

        #file_handle.write(hexStr_to_str(cipher128))
